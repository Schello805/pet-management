document.addEventListener('DOMContentLoaded', () => {
    ...
    function fuegeDokumentHinzu(event) {
        event.preventDefault();
        const fileInput = dokumenteForm.querySelector('#dokument-upload');
        ...
        if (file) {
            ...
            fileInput.value = '';
            // Hier können Sie den Code hinzufügen, um die Datei auf dem Server zu speichern
        }
    }

    function speichereHaustierDaten(id) {
        const haustierForm = document.getElementById(`bearbeite-haustier-form-${id}`);
        const haustier = haustiere.find(h => h.id === id);

        haustier.name = haustierForm.querySelector(`#name-${id}`).value;
        haustier.geburtsdatum = haustierForm.querySelector(`#geburtsdatum-${id}`).value;
        haustier.geschlecht = haustierForm.querySelector(`#geschlecht-${id}`).value;
        haustier.letzteImpfung = haustierForm.querySelector(`#letzteImpfung-${id}`).value;
        haustier.letzteImpfungBeschreibung = haustierForm.querySelector(`#letzteImpfungBeschreibung-${id}`).value;
        haustier.naechsteImpfung = haustierForm.querySelector(`#naechsteImpfung-${id}`).value;
        haustier.naechsteImpfungBeschreibung = haustierForm.querySelector(`#naechsteImpfungBeschreibung-${id}`).value;
        haustier.rasse = haustierForm.querySelector(`#rasse-${id}`).value;
        haustier.foto = haustierForm.querySelector(`#foto-preview-${id}`).src;

        zeigeHaustierListe();
    }

    function zeigeSeite(sectionId) {
        for (const key in sections) {
            if (sections.hasOwnProperty(key)) {
                sections[key].classList.toggle('d-none', key !== sectionId);
            }
        }
    }

    document.getElementById('link-haustier-liste').addEventListener('click', () => zeigeSeite('haustier-liste-section'));
    document.getElementById('link-neues-haustier').addEventListener('click', () => zeigeSeite('neues-haustier-section'));
    document.getElementById('link-profil').addEventListener('click', () => zeigeSeite('profil-section'));
    document.getElementById('link-dokumente').addEventListener('click', () => zeigeSeite('dokumente-section'));
    ...
    zeigeHaustierListe(); // Initiale Anzeige der Haustierliste
});
